...
if __name__ == "__main__":
    ...
    # fine-tune model
    linear_cls = nn.Sequential(nn.Linear(num_features, n_classes))
    linear_cls = linear_cls.to(device)

    # loss / optimizer
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(params=linear_cls.parameters(), lr=args.learning_rate)
...
#         print("### Calculating final testing performance ###")
        ...
        print(f"test accuracy: " + "\t".join([f"{k}: {np.array(v).mean()}" for k, v in metrics.items()]))
    ...
    torch.save(linear_cls.state_dict(), "./linear_classfication.pt")
